...
const tableColumns = [
  ...
  { key: "name", label: t("configuration.subOrganization.name") },
  { key: "officer", label: t("common.officer"), width: "220px" },
  { key: "sites", label: t("dashboard.statLabels.site"), width: "150px" },
...
async function loadSubOrganisasi() {
  ...
  try {
    ...
    toast.error(t("common.loadFailed", { entity: t("configuration.subOrganization.countEntity") }))
  } finally {
    loading.value = false
  }
}
...
async function saveSub() {
  ...
  try {
    ...
    closeModal()
    toast.success(t("common.saveSuccess", { entity: t("configuration.subOrganization.countEntity") }))

  } catch (err) {
    console.error("Failed to save sub organisasi:", err.response?.data || err)
    toast.error(t("common.saveFailed", { entity: t("configuration.subOrganization.countEntity") }))
  } finally {
    saving.value = false
  }
}
...
async function confirmDelete() {
  ...
  try {
    ...
    toast.success(t("configuration.subOrganization.deleteSuccess"))

  } catch (err) {
    console.error("Delete failed:", err)
    toast.error(t("common.deleteFailed", { entity: t("configuration.subOrganization.countEntity") }))
  }
}
...
<template>
  <div>
    ...
    <PageToolbar
      ...
      :action-text="t('configuration.subOrganization.add')"
      @action="openAddModal"
    >
    ...
    <ConfigTable
      ...
      :loading="loading"
      :empty="paginatedSubs.length === 0"
      :empty-message="t('configuration.subOrganization.empty')"
      :empty-action-text="search.trim() ? '' : t('configuration.subOrganization.add')"
      min-width="780px"
    ...
    <FormModal
      :show="showModal"
      :eyebrow="isEditMode ? t('configuration.shared.editData') : t('configuration.shared.addData')"
      :title="isEditMode ? t('configuration.subOrganization.edit') : t('configuration.subOrganization.add')"
      @close="closeModal"
    >
...
/>
    ...
    <ConfirmActionModal
      ...
      :title="t('common.deleteTitle', { name: selectedSubRecord?.nama || t('common.emptyValue') })"
