...
describe("PengurusanAkaun.vue", () => {
  ...
  it("menjana kata laluan sementara apabila akaun diaktifkan semula", async () => {
    api.get.mockResolvedValue({
      data: [
        {
          id: 2,
    ...
    const editButtons = wrapper.findAll(".clickable-row .ui-icon-button")
    await editButtons[0].trigger("click")
    await wrapper.find('input[type="checkbox"]').trigger("click")

    const confirmationInput = wrapper.find(".confirm-action-modal__input")
    await confirmationInput.setValue("Aktifkan")
    await wrapper
      .find(".confirm-action-modal__actions .ui-button--primary")
      .trigger("click")

    await Promise.resolve()
    await wrapper.vm.$nextTick()

    expect(api.put).toHaveBeenCalledWith("/auth/users/2/activate")
    expect(wrapper.text()).toContain("Akaun Berjaya Diaktifkan Semula")
    expect(wrapper.text()).toContain("inactive.user1")
    expect(wrapper.text()).toContain("NewSecure1!")
  })
