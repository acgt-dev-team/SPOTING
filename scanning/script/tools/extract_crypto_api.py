...
def read_header_file(header_path: Path):
    """
    Read a header file and return its contents.
    """
...
def is_candidate_function(line: str) -> bool:
    ...
    # Only public exported APIs
    if "WOLFSSL_API" not in line:
        return False
...
def main():
    header_dir = Path(r"C:\Users\Admin\Downloads\wolfssl-master\wolfssl-master\wolfssl\wolfcrypt")

    if not header_dir.exists():
        print(f"Directory not found: {header_dir}")
        return

    header_files = find_header_files(header_dir)
    ...
    for header in header_files:
        ...
        lines = read_header_file(header)

        for line in lines:

            if not is_candidate_function(line):
                continue

            name = extract_function_name(line)

            if not is_public_api(name):
                continue

            # Store the first header where this API was found
    ...
    with open("wolfssl_api_database.json", "w", encoding="utf-8") as file:
        json.dump(export_data, file, indent=4)

    print("\nJSON exported to wolfssl_api_database.json")
