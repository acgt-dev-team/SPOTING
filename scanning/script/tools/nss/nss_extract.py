...
def main():
    header_dir = Path(r"C:\AGCT\library-sources\nss-3.125\nss\lib")
    ...
    for header_name in sorted(api_database):
        count = len(api_database[header_name])
        total += count
        print(f"{header_name:<35} {count}")
    ...
    with open("nss_api_database.json", "w", encoding="utf-8") as file:
        json.dump(export_data, file, indent=4)

    print("\nJSON exported to nss_api_database.json")
